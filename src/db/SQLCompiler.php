...
class SQLCompiler {
	...
	function as_sql($select) {
		$model = $this->query->model;
		$where = '';
		if (count($this->query->where)>0) {
			$where = 'AND '.join(" AND ",$this->query->where);
		}
	...
	function tables($model,$order_by) {
		$tables = array();
		$joins = array();
		...
		return array($tables,join(' AND ',$joins),join(',',$order));
	}
	function addTable($model,&$tables,&$joins,&$order,$a,$tindex,$desc) {
