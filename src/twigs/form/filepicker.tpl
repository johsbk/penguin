{% extends "ajax.tpl" %}
{% block head %}
<script type="text/javascript" src="{{ TEMPLATE_PATH }}swfupload/swfupload.js"></script>
...
$(function () {
	var settings_object = {
			...
			flash_url : TEMPLATE_PATH+"/media/swfupload/swfupload.swf",
			flash9_url : TEMPLATE_PATH+"/media/swfupload/swfupload_fp9.swf",
...
function fpClick() {
	if ($(this).attr("data-path")=='dir') {
		...
		Filepicker.select($(this).attr("data-path")+"/"+$(this).attr("data-name"));
	}
}
function preLoad() {
...
{% for file in files %}
	...
	{% endif %}
	<td data-path="{{ file.path }}" data-name="{{ file.name }}" data-type="{{ file.type }}">
		<img src="{{ file.picture }}" /><br />
		{{ file.name }}
	</td>
{% endfor %}
</tr></table>
