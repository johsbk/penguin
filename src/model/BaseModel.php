...
abstract class BaseModel {
	...
	function __call($method, $params) {
		...
		if ($class = static::has($method)) {
			list($m,$f) = $class;
			$dict = $params[0];
			if (!isset($dict['where'])) $dict['where'] = array();
	...
	static function addHas($class,$field=false) {
	...
	static function init() {
		...
		if (!Functions::nz(static::$_initialized[$class],false)) {
			...
			static::localinit();
			static::build();
			if(isset($_GET['checkintegrity'])) self::checkIntegrity();		
		}
	}
	static function getForeignKeyModel($field) {
		self::init();
	...
	static protected function localinit() {}
	...
	public static function find($find,$dict=array()) {
		...
		if ($count) {
			...
			$q = DB::fetchOne($sql);
			if ($q) return new static($q);
			else return false;
		} else {
			$rs = DB::fetch($sql);
			$ar = array();
			while ($row = $rs->next()) $ar[] = new static($row);
			return $ar;	
		}
	}
	...
	static function formDict($dict=array()) {
		...
		foreach(static::getFields() as $f) {
			if (($showfields and (array_key_exists($f->name, $showfields) or in_array($f->name,$showfields))) or !$showfields) {
				...
				if ($f instanceof ForeignKeyField) {
					...
					$foptions = array();
					foreach ($f->options as $option) $foptions[] = array("name"=>$option);
